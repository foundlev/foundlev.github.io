...
// При прокрутке страницы выполнить функцию
window.onscroll = function() {scrollFunction()};

function scrollFunction() {
  if (document.body.scrollTop > 1000 || document.documentElement.scrollTop > 1000) {
    mybutton.style.display = "block"; // Показать кнопку
  } else {
    mybutton.style.display = "none"; // Скрыть кнопку
  }
}

// При нажатии на кнопку прокрутить страницу наверх
...
// При клике на ссылку открыть модальное окно и воспроизвести видео
document.querySelector('a[href="/assets/video/baltwave_bot_video.mp4"]').addEventListener('click', function(event) {
  event.preventDefault(); // Предотвратить переход по ссылке
  modal.style.display = "block";
  modalVideo.src = this.href; // Установить источник видео
  modalVideo.play(); // Воспроизвести видео
});

// Клик по крестику закрывает модальное окно и останавливает видео
closeModal.onclick = function() {
  modal.style.display = "none";
  modalVideo.pause(); // Остановить видео
  modalVideo.currentTime = 0; // Сбросить время видео
}
...
function updateIcons() {
    ...
    // Получаем все изображения с классом 'theme-icon'
    document.querySelectorAll('.theme-icon').forEach(img => {
        let src = img.src;
        let basePath = src.substring(0, src.lastIndexOf("/"));  // Получаем базовый путь до папки с изображением
        let fileName = src.substring(src.lastIndexOf("/") + 1); // Извлекаем имя файла
        let fileExtension = fileName.substring(fileName.lastIndexOf('.')); // Извлекаем расширение файла
        let baseFileName = fileName.substring(0, fileName.lastIndexOf('.')); // Извлекаем базовое имя файла без расширения

        // Обновляем src в зависимости от темы
        if (isDarkMode) {
            // Добавляем '_white' перед расширением файла для темной темы
            img.src = `${basePath}/${baseFileName}_white${fileExtension}`;
        } else {
            // Убираем '_white' для светлой темы, предполагая, что основное имя файла без '_white'
            img.src = `${basePath}/${baseFileName.replace('_white', '')}${fileExtension}`;
        }
    });
}

// Вызываем функцию при загрузке страницы и при изменении предпочтений темы
document.addEventListener('DOMContentLoaded', updateIcons);
window.matchMedia('(prefers-color-scheme: dark)').addListener(updateIcons);
